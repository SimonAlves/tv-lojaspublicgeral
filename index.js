...
let campanhas = [
    // 0. CUPOM DA SORTE (O ÚNICO QUE TEM SORTEIO)
    { 
        id: 0, 
        tipo: 'foto', 
        arquivo: "cupomdasorte.jpg", // <--- Sua imagem de sorteio aqui
        nome: "Tente a Sorte!", 
        qtd: 5, // Estoque do prêmio R$ 50
        ativa: true, 
    ...
    // 1. PREMIER (VOLTOU AO NORMAL - 10% OFF)
    { 
        id: 1, 
        tipo: 'foto', 
        arquivo: "slide1.jpg", 
        ...
        prefixo: 'PREMIER',
        ehSorteio: false, // <--- GARANTIDO
        totalResgates: 0,
        resgatesPorHora: new Array(24).fill(0),
        ultimoCupom: "Nenhum",
        ultimaHora: "--:--"
    },
    // 2. SPECIAL DOG (GARANTIDO)
    { 
        id: 2, 
        tipo: 'foto', 
        arquivo: "slide2.jpg", 
...
<div id="telaPegar" style="margin-top:50px;">
    ...
    <p style="color:#666;">Aguarde um instante</p>
</div>
<div id="telaVoucher" style="display:none">
    ...
    <div class="ticket-white" id="ticketContainer">
        <div style="padding:25px">
            ...
            <h1 id="voucherNome" style="font-size:24px;margin:5px 0">...</h1>
            <div style="background:#f9f9f9;border:2px dashed #ccc;padding:15px;margin:20px 0">
                <div class="codigo-texto" id="codGerado">...</div>
            </div>
            <p style="font-size:12px;color:gray">Gerado em: <span id="dataHora" style="font-weight:bold"></span><br>Válido hoje.</p>
        </div>
    </div>
...
const salvo=localStorage.getItem('polipet_cupom_final_v3');
const dataSalva=localStorage.getItem('polipet_data_final_v3');
...
socket.on('sucesso',dados=>{
    ...
    localStorage.setItem('polipet_cupom_final_v3',JSON.stringify(dados));
    localStorage.setItem('polipet_data_final_v3',agora.toLocaleDateString('pt-BR'));
...
function mostrarVoucher(dados){
    ...
    if(dados.isGold){
        ...
        document.getElementById('tituloParabens').style.color="#003399";
    }
}
</script></body></html>`;
    ...
    socket.on('resgatar_oferta', (id) => {
        ...
        if (camp && camp.qtd > 0) {
            ...
            if (camp.ehSorteio) {
                ...
                // 1% de chance de ganhar R$ 50
                if (sorte > 99) { 
                    isGold = true; nomeFinal = "🏆 GANHOU R$ 50,00 OFF"; cor1='#FFD700'; prefixo="MAX";
                } else {
                    // 99% de chance de ganhar 5%
                    nomeFinal = "Ganhou: 5% OFF"; prefixo="MIN";
                }
            }
